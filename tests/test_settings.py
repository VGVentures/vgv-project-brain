...
def test_settings_optional_connectors_default_none(monkeypatch, tmp_path):
    monkeypatch.chdir(tmp_path)  # no .env file here — prevents loading real credentials
    monkeypatch.setenv("SUPABASE_URL", "https://test.supabase.co")
    monkeypatch.setenv("SUPABASE_SERVICE_ROLE_KEY", "svc-key")
    monkeypatch.setenv("SUPABASE_ANON_KEY", "anon-key")
...
def test_settings_raises_on_missing_required(monkeypatch, tmp_path):
